...
- **Key modules:**
  ...
  - `server/influence.py` — scrapes Politico Influence newsletter using Playwright headless Chromium (to bypass Cloudflare). Extracts bold entities from HTML, detects section headings (all-caps bolded text) as metadata, classifies entities as person/organization/unknown via NLP heuristics (contextual patterns, name structure), detects affiliations via regex, builds co-occurrence relationships with section-level cross-paragraph edges at half weight. Parses "New Lobbying Registrations" and "New Lobbying Terminations" sections for `Company: Client` pairs, creating organization entities with `lobbying_registration` or `lobbying_termination` relationship type at edge weight 1.5. Supports user type overrides (`user_override` flag on entities preserved across reprocessing). Strips boilerplate (byline, date, archive links) from body_text. Requires `LD_LIBRARY_PATH` for `libgbm` (handled in `start.sh`).
