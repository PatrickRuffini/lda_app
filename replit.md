...
- **Key modules:**
  ...
  - `server/influence.py` — scrapes Politico Influence newsletter using Playwright headless Chromium (to bypass Cloudflare). Extracts bold entities from HTML, detects section headings (all-caps bolded text) as metadata, classifies entities as person/organization/unknown via NLP heuristics (contextual patterns, name structure), detects affiliations via regex, builds co-occurrence relationships with section-level cross-paragraph edges at half weight. Supports user type overrides (`user_override` flag on entities preserved across reprocessing). Requires `LD_LIBRARY_PATH` for `libgbm` (handled in `start.sh`).
